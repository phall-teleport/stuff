...
struct Session: Codable, Identifiable, Hashable {
    ...
    /// Path of the Codex rollout file relative to ~/.codex, so it can be put back.
    var codexSessionRel: String = ""
    /// Set when a session was moved to a beam without the agent's conversation
    /// file. The next Claude turn must start a new conversation (--session-id,
...
/// A session found under `<prefix>/sessions/<id>/` in the configured GitHub
/// repo and branch, which can be imported and continued in a beam.
struct RemoteSession: Identifiable, Hashable {
    ...
    var hasMemory: Bool
    /// Claude's own conversation file was synced, so `--resume` can continue it.
    var hasClaudeSession: Bool
    var isCodex: Bool
    /// Present when the repo copy has meta.json (sessions synced by newer builds).
    var meta: Session?

    var resumable: Bool { hasClaudeSession || (isCodex && meta?.codexSessionRel.isEmpty == false) }
}

enum ClaudePaths {
    /// Claude Code names a project's folder under ~/.claude/projects after the
    /// working directory with every non-alphanumeric character turned into "-":
    /// /home/beams/work → -home-beams-work (verified in a beam).
    static func projectSlug(_ workDir: String) -> String {
        String(workDir.map { $0.isASCII && ($0.isLetter || $0.isNumber) ? $0 : "-" })
    }
}
