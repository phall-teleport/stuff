...
final class StreamReader {
    ...
    /// Waits for the reader to reach EOF, but not forever: a child's grandchild
    /// (e.g. git's `gh auth git-credential` helper) can inherit the pipe and
    /// keep its write end open after the child exits, so EOF never arrives.
    /// After a short grace period we close the read handle, which forces the
    /// blocked `availableData` to return and the thread to finish.
    private func waitBounded() {
        if doneSem.wait(timeout: .now() + 3) == .timedOut {
            try? handle.close()          // unblocks availableData → thread exits
            _ = doneSem.wait(timeout: .now() + 2)
        }
    }
